...
# 
# Program name: simsummary_proportion.R
# 
...
#          Run this after main.sh and summary.R finishes for all sample sizes with resampling
#          Combine formatted data across level of heterogeneity and percentile. 
# 
#          This was run on high-performance cluster via simsummary_proportion.sh
# ------------------------------------------------------------------
...
summarize <- function(input, n, beta, percentiles){
  ...
  vhats.dhat <- read_csv(paste0("./simulations/proportion/outputs/simmain_CV_results_vhats.dhat_n", n, "_stratified", n.fold, "folds_beta", 
  ...
  vs.dhat <- read_csv(paste0("./simulations/proportion/outputs/simmain_CV_results_vs.dhat_n", n, "_stratified", n.fold, "folds_beta", 
                               paste0(beta, collapse = "-"), "_perc", paste0(percentiles, collapse = "-"), ".csv"))
  ...
  dhats <- read_csv(paste0("./simulations/proportion/outputs/simmain_CV_results_dhats_n", n, "_stratified", n.fold, "folds_beta", 
                             paste0(beta, collapse = "-"), "_perc", paste0(percentiles, collapse = "-"), ".csv"))
  ...
  dat.accuracy <- dhat.cvs2 %>% group_by(beta, percentiles, n.subset, method, fold, iteration) %>% dplyr::summarise(accuracy = sum(dhat == d)/n(), accuracy.wo.neutral = sum(dhat[neutral == 0] == d[neutral == 0])/sum(neutral == 0))
  dat.accuracy.summary <- dat.accuracy %>% group_by(beta, percentiles, n.subset, method) %>% 
  ...
  levhte <- "medium"
  if(all(percentiles == c(0, 0.55, 0.65, 0.75, 0.85, 1))) pergr <- "asymm 55-30-15"
  if(all(percentiles == c(0, 0.55, 0.65, 0.7, 0.85, 1))) pergr <- "asymm 55-15-15-15"
  if(all(percentiles == c(0, 0.1, 0.4, 0.6, 0.9, 1))) pergr <- "symm 10-30-20-30-10"
  if(all(percentiles == c(0, 0.1, 0.25, 0.75, 0.9, 1))) pergr <- "symm 10-15-50-15-10"
  if(all(percentiles == c(0, 0.2, 0.4, 0.6, 0.8, 1))) pergr <- "symm 20-20-20-20-20"
  
  # Rename data sets to export with tags
  assign(paste0("cvs2_n", n, "_", levhte, "_", pergr), cvs2)
  assign(paste0("true.cvs2_n", n, "_", levhte, "_", pergr), true.cvs2)
  assign(paste0("all.cvs3_n", n, "_", levhte, "_", pergr), all.cvs3)
  assign(paste0("all.cvs3.wide_n", n, "_", levhte, "_", pergr), all.cvs3.wide)
  assign(paste0("dhat.agreement_n", n, "_", levhte, "_", pergr), dhat.agreement)
  assign(paste0("dat.accuracy_n", n, "_", levhte, "_", pergr), dat.accuracy)
  assign(paste0("dat.accuracy.summary_n", n, "_", levhte, "_", pergr), dat.accuracy.summary)
  
  save(file = paste0("./simulations/proportion/outputs/simsummary_n", n, "_dataplot_", levhte, "_", pergr, ".RData"), list = ls()[grepl("_", ls())])
}

for (i in 1:5){
  print(beta_group[[i]])
  print(percentile_group[[i]])
  summarize(input, ns, beta_group[[i]], percentile_group[[i]])
}

# These output files need to be moved or copied to "./intermediate_results/simulations_proportion/" to be run by "simulation_analysis.R"
